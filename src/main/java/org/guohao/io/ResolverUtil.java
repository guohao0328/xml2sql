...
import org.guohao.logging.Log;
import org.guohao.logging.LogFactory;

/**
 * <p>ResolverUtil is used to locate classes that are available in the/a class path and meet
 * arbitrary conditions. The two most common conditions are that a class implements/extends another
 * class, or that is it annotated with a specific annotation. However, through the use of the {@link
 * Test} class it is possible to search using arbitrary conditions.</p>
 *
 * <p>A ClassLoader is used to locate all locations (directories and jar files) in the class
 * path that contain classes within certain packages, and then to load those classes and check them.
 * By default the ClassLoader returned by {@code Thread.currentThread().getContextClassLoader()} is
